...
public class RequestAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RequestAdapter(FragmentManager fragmentManager, Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch(viewType){
            case ITEM_UPPER:
                view = layoutInflater.inflate(R.layout.layout_request,parent,false);
                vh =  new UpperViewHolder(view);
                break;
            case ITEM_BOTTOM:
               view = layoutInflater.inflate(R.layout.layout_request_bottom,parent,false);
    ...
    public void bottomView(final BottomViewHolder holder){
        ((RequestActivity)context).getSupportFragmentManager().beginTransaction().add(R.id.eventDonorContainerFrame,new com.example.denisprawira.ta.UI.Fragment.Request.EventFragment()).commit();

        holder.bubbleNavigationConstraintView.setNavigationChangeListener(new BubbleNavigationChangeListener() {
            @Override
            public void onNavigationChanged(View view, int position) {
                if(position==0){
                    ((RequestActivity)context).getSupportFragmentManager().beginTransaction().replace(R.id.eventDonorContainerFrame,new EventFragment()).commit();
                }else if(position==1){
                    ((RequestActivity)context).getSupportFragmentManager().beginTransaction().replace(R.id.eventDonorContainerFrame,new DonorFragment()).commit();
                }
            }
        });
    }
